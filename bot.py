...
def handle_start(message):
    ...
    # Secure check for text split
    if not message.text:
        return
        
    text_args = message.text.split()

    # Agar user link se aaya hai (Matalb argument maujood hai)
    if len(text_args) > 1:
        file_id = text_args[1]
        ...
        # Agar normal user ya admin bina link ke start karta hai
        if user_id == ADMIN_ID:
...
def send_requested_file(chat_id, file_id):
    if file_id in file_database:
        ...
        # Agar bot restart hone se database saaf ho gayi, toh user ko pata chalega
